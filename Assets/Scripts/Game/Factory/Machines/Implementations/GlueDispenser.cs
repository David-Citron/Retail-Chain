...
public class GlueDispenser : Machine
{
    ...
    protected override void OnStart()
    {
        AddInteraction(new Interaction(GetTag(), () => PressedKey(ActionType.Interaction) && isPlayerNear, i => StartInteraction(), new Hint[] {
            new Hint(Hint.GetHintButton(KeyCode.Space) + " TO GET CANISTER", () => !PlayerPickUp.IsHodlingItem() && machineState == MachineState.Ready),
            new Hint(Hint.GetHintButton(KeyCode.Space) + " TO FILL GLUE DISPENSER", () => PlayerPickUp.IsHodlingItem()),
